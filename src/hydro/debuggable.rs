...
impl Instruction {
  pub fn debug(&self, module: &Module, context: &mut ExecutionContext, debug_context: &mut DebugContext) -> Result<bool, Exception> {
    match self {
      ...
      Value::FunctionPointer(func_pointer) => {
        ...
        match target_module.functions.get(func_pointer.function.clone().as_str()) {
          Some(target_function) => {
            ...
            let return_value = target_module.debug(func_pointer.function, arguments, Some(Box::new(context.clone())), debug_context);
            match return_value {
